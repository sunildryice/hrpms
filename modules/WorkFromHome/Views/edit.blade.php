        ...
        <div class="card-body">
            <form action="{{ route('wfh.requests.update', $workFromHome->id) }}" id="wfhRequestEditForm" method="POST"
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="reason" name="reason" rows="3" required>{{ old('reason', $workFromHome->reason) }}</textarea>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control" id="send_to" name="send_to" required>
                        ...
                        @foreach ($supervisors as $id => $fullName)
                            <option value="{{ $id }}" @if (old('send_to', $workFromHome->approver_id) == $id) selected @endif>
                                {{ $fullName }}
                            </option>
                        @endforeach
                    </select>
