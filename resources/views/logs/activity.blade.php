@extends('layouts.container')

@section('title', 'Activity Logs')

@section('page_js')
    <script type="text/javascript">
        $(document).ready(function () {
            $('#navbarVerticalMenu').find('#activity-logs-menu').addClass('active');

            var oTable = $('#activityLogTable').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('activity.logs.index') }}",
                columns: [
                    {data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false},
                    {data: 'user', name: 'user'},
                    {data: 'ip_address', name: 'ip_address'},
                    {data: 'route', name: 'route'},
                    {data: 'agent', name: 'agent'},
                    {data: 'created_at', name: 'created_at'},
                ]
            });
        });
    </script>
@endsection
@section('page-content')

    <div class="container-fluid">
        <div class="pb-3 mb-3 border-bottom">
            <div class="d-flex flex-column flex-lg-row align-items-start align-items-lg-center gap-2">
            <div class="brd-crms flex-grow-1">
                <nav aria-label="breadcrumb">
                        <ol class="breadcrumb m-0">
                            <li class="breadcrumb-item"><a href="{{ route('dashboard.index') }}"
                                                           class="text-decoration-none text-dark">Home</a></li>
                            {{--                            <li class="breadcrumb-item"><a href="#" class="text-decoration-none">HR</a></li>--}}
                            <li class="breadcrumb-item" aria-current="page">@yield('title')</li>
                        </ol>
                    </nav>
                    <h4 class="m-0 lh1 mt-1 fs-6 text-uppercase fw-bold text-primary">@yield('title')</h4>
                </div>
            </div>
        </div>
    </div>

    <div class="container-fluid">
        <div class="card shadow-sm border rounded c-tabs-content active">
            <div class="card-body">
                <table class="table" id="activityLogTable">
                    <thead>
                    <tr>
                        <th>{{ __('label.sn') }}</th>
                        <th scope="col">{{ __('label.name') }}</th>
                        <th scope="col">{{ __('label.ip-address') }}</th>
                        <th scope="col">{{ __('label.route') }}</th>
                        <th scope="col">{{ __('label.agent') }}</th>
                        <th scope="col">{{ __('label.datetime') }}</th>
                    </tr>
                    </thead>
                    <tbody>
                    </tbody>
                </table>

            </div>
        </div>
    </div>
@stop
