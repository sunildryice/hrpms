<div class="card-header fw-bold">Add Educational Details</div>
<form class="needs-validation" action="{{ route('employees.education.store', $employee->id) }}" method="post"
    ...
    <div class="card-body">
        ...
        <div class="row mb-2">
            ...
            <div class="col-lg-9">
                <input type="text" class="form-control @if($errors->has('edu_institution')) is-invalid @endif" id="validationinstitution" value="{{ old('edu_institution') }}"
                    placeholder="Institution" name="edu_institution" autofocus>
                @if($errors->has('edu_institution'))
                    <div class="fv-plugins-message-container invalid-feedback">
                        <div data-field="edu_institution">{!! $errors->first('edu_institution') !!}</div>
                    </div>
                @endif
            </div>
        </div>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function (e) {
            ...
            $(form).on('change', '[name="education_level_id"]', function (e){
                fv.revalidateField('education_level_id');
            }).on('change', '[name="passed_year"]', function (e){
                fv.revalidateField('passed_year');
            });
        });
    </script>
@endpush
