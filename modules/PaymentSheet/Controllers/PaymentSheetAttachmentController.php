...
use Modules\PaymentSheet\Requests\PaymentSheetAttachment\StoreRequest;
use Modules\PaymentSheet\Requests\PaymentSheetAttachment\UpdateRequest;
use Modules\PaymentSheet\Repositories\PaymentSheetAttachmentRepository;
use Yajra\DataTables\DataTables;


class PaymentSheetAttachmentController extends Controller
{
    private $attachments;
    private $destinationPath;
    public function __construct(
        PaymentSheetAttachmentRepository $attachments
    )
    {
        $this->attachments = $attachments;
        $this->destinationPath = 'paymentSheetAttachment';
    }
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $btn .= route('payment.sheets.attachment.edit', $row->id) . '" rel="tooltip" title="Edit"><i class="bi-pencil-square"></i></a>';

                    $btn .= '&emsp;<a href = "javascript:;" class="btn btn-danger btn-sm delete-record" ';
                    $btn .= 'data-href="' . route('payment.sheets.attachment.destroy', $row->id) . '">';
    ...
    public function create($paymentSheetId)
    {
        ...
        return view('PaymentSheet::PaymentSheetAttachment.create', $array);
    }

    /**
     * Store a newly created resource in storage.
     *
     * @param  \Modules\PaymentSheet\Requests\PaymentSheetAttachment\StoreRequest  $request
     * @param mixed $paymentSheetId
    ...
    public function store(StoreRequest $request, $paymentSheetId)
    {
        ...
        $inputs['payment_sheet_id'] = $paymentSheetId;
        DB::beginTransaction();
        try {
            if ($request->file('attachment')) {
                $filename = time().'_'.random_int(1000, 9999).'_payment_sheet_attachment'.$request->file('attachment')->getClientOriginalExtension();
    ...
    public function show($attachmentId)
    {
        ...
        return view('PaymentSheet::PaymentSheetAttachment.show', $array);
    }
    ...
    public function edit($attachmentId)
    {
        ...
        return view('PaymentSheet::PaymentSheetAttachment.edit', $array);
    }
