@extends('layouts.container')

@section('title', 'Report : Purchase Requests')

@section('page_js')
<script type="text/javascript">

        let start_date  = '';
        let end_date    = '';
        let pr_number   = '';
        let office      = '';
        let requester   = '';
        let particulars = '';

        $(document).ready(function() {

            $('#navbarVerticalMenu').find('#purchase-request-report-menu').addClass('active');

            var oTable = $('#purchaseRequestReportTable').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('report.purchase.request.index') }}",
                    type: 'POST',
                    data: function(d) {
                        d.start_date    = start_date;
                        d.end_date      = end_date;
                        d.pr_number     = pr_number;
                        d.office        = office;
                        d.requester     = requester;
                        d.particulars   = particulars;
                    }
                },
                columns: [{
                        data: 'DT_RowIndex',
                        name: 'DT_RowIndex',
                        orderable: false,
                        searchable: false
                    },
                    {
                        data: 'purchase_request_number',
                        name: 'purchase_request_number',
                        orderable: true,
                        searchable: true
                    },
                    {
                        data: 'office',
                        name: 'office',
                        orderable: true,
                        searchable: true
                    },
                    {
                        data: 'requested_date',
                        name: 'requested_date',
                        orderable: true,
                        searchable: true
                    },
                    {
                        data: 'requested_by',
                        name: 'requested_by',
                        orderable: true,
                        searchable: true
                    },
                    {
                        data: 'required_date',
                        name: 'required_date',
                        orderable: true,
                        searchable: true
                    },
                    {
                        data: 'particulars',
                        name: 'particulars',
                        orderable: true,
                        searchable: true
                    },
                    {
                        data: 'quantity',
                        name: 'quantity',
                        orderable: true,
                        searchable: true
                    },
                    {
                        data: 'tentative_cost',
                        name: 'tentative_cost',
                        orderable: true,
                        searchable: true
                    },
                    {
                        data: 'project',
                        name: 'project',
                        orderable: true,
                        searchable: true
                    },
                    {
                        data: 'activity_code',
                        name: 'activity_code',
                        orderable: true,
                        searchable: true
                    },
                    {
                        data: 'account_code',
                        name: 'account_code',
                        orderable: true,
                        searchable: true
                    },
                    {
                        data: 'donor_code',
                        name: 'donor_code',
                        orderable: true,
                        searchable: true
                    },
                    {
                        data: 'remarks',
                        name: 'remarks'
                    },
                ],
                scrollX: true
            });

            $('#btn_search').on('click', function(e) {
                if ($('#start_date').val()) {
                    let start = new Date($('#start_date').val());
                    start_date = start.getTime();
                }

                if ($('#end_date').val()) {
                    let end = new Date($('#end_date').val());
                    end_date = end.getTime();
                }

                if ($('#pr_number').val()) {
                    pr_number = $('#pr_number').val();
                }

                if ($('#office').val()) {
                    office = $('#office').val();
                }

                if ($('#requester').val()) {
                    requester = $('#requester').val();
                }

                if ($('#particulars').val()) {
                    particulars = $('#particulars').val();
                }

                if (start_date > end_date) {
                    $('#error_message').html('\'From\' date cannot be greater than \'To\' date.');
                    return;
                } else {
                    $('#error_message').html('');
                    $('#error_message').hide();
                }

                $('#btn_export').attr('href', '');
                $('#btn_export').attr('href', $('#btn_export').attr('href') +
                    '/report/purchase/request/export?start_date=' + start_date + '&end_date=' +
                    end_date + '&office=' + office + '&pr_number=' + pr_number + '&requester=' + requester + '&particulars=' + particulars);

                oTable.ajax.reload();
            });

            $('[name=start_date]').datepicker({
                language: 'en-GB',
                autoclose: true,
                format: 'yyyy-mm-dd'
            }).on('change', function (e) {
                $(this).datepicker('hide');
            });

            $('[name=end_date]').datepicker({
                language: 'en-GB',
                autoclose: true,
                format: 'yyyy-mm-dd'
            }).on('change', function (e) {
                $(this).datepicker('hide');
            });

            $('#btn_reset').on('click', function(e) {
                start_date  = '';
                end_date    = '';
                pr_number   = '';
                office      = '';
                requester   = '';
                particulars = '';
                $("input[type=text]").removeAttr('value');
                $('#office').val('').trigger("change");
                $('#requester').val('').trigger("change");
                $('#particulars').val('').trigger("change");
            });
        });

        function resetValue(name){
            let value = null;
            eval(name + "=" + value + ";");
        }


    </script>
@endsection
@section('page-content')

        <div class="pb-3 mb-3 border-bottom">
            <div class="d-flex flex-column flex-lg-row align-items-start align-items-lg-center gap-2">
            <div class="brd-crms flex-grow-1">
                <nav aria-label="breadcrumb">
                        <ol class="breadcrumb m-0">
                            <li class="breadcrumb-item"><a href="{{ route('dashboard.index') }}"
                                    class="text-decoration-none text-dark">Home</a></li>
                            {{-- <li class="breadcrumb-item"><a href="#" class="text-decoration-none">HR</a></li> --}}
                            <li class="breadcrumb-item" aria-current="page">@yield('title')</li>
                        </ol>
                    </nav>
                    <h4 class="m-0 lh1 mt-1 fs-6 text-uppercase fw-bold text-primary">@yield('title')</h4>
                </div>
                <div class="add-info justify-content-end">
                    <a href="{{ route('report.purchase.request.export') }}" id="btn_export" class="btn btn-primary btn-sm">
                        Export
                    </a>
                </div>
            </div>
        </div>
        <div class="card c-tabs-content active" id="employee-table" style="overflow: auto;">
            <div class="card-body">
                <form>
                    <div class="row mb-2" style="align-items: flex-end">
                        <div class="col-md-2">
                            <label class="form-label" for="start_date">From</label>
                            <input class="form-control" type="text" name="start_date" id="start_date">
                        </div>
                        <div class="col-md-2">
                            <label class="form-label" for="end_date">To</label>
                            <input class="form-control" type="text" name="end_date" id="end_date">
                        </div>
                        <div class="col-md-2">
                            <label class="form-label" for="pr_number">PR No.</label>
                            <input class="form-control" type="text" name="pr_number" id="pr_number">
                        </div>
                        <div class="col-md-2">
                            <label class="form-label" for="office">Office</label>
                            <select class="form-control select2" name="office" id="office">
                                <option value="" selected onclick="resetValue('office')">Select Office...</option>
                                @foreach ($offices as $office)
                                    <option value="{{ $office->id }}">{{ $office->office_name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-2">
                            <label class="form-label" for="requester">Requested by</label>
                            <select class="form-control select2" name="requester" id="requester">
                                <option value="" selected onclick="resetValue('requester')">Select requester...</option>
                                @foreach ($employees as $employee)
                                    @if ($employee->user)
                                        <option value="{{ $employee->user->id }}">{{ $employee->getFullName() }}</option>
                                    @endif
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-2">
                            <label class="form-label" for="particulars">Particulars</label>
                            <select class="form-control select2" name="particulars" id="particulars">
                                <option value="" selected onclick="resetValue('particulars')">Select item...</option>
                                @foreach ($items as $item)
                                    <option value="{{ $item->id }}">{{ $item->title }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>

                    <span class="text-danger" id="error_message"></span>

                    <div class="row mb-2">
                        <div class="col d-flex justify-content-end">
                            <button type="button" id="btn_search" class="btn btn-primary btn-sm m-1">Search</button>
                            <button type="reset" id="btn_reset" class="btn btn-secondary btn-sm m-1">Reset</button>
                        </div>
                    </div>

                </form>
                <div class="table-responsive">
                    <table class="table" id="purchaseRequestReportTable">
                        <thead class="bg-light">
                            <tr>
                                <th>{{ __('label.sn') }}</th>
                                <th>PR No.</th>
                                <th>Office</th>
                                <th>Requested Date</th>
                                <th>Requested By</th>
                                <th>Required Date</th>
                                <th>Particulars</th>
                                <th>Quantity</th>
                                <th>Tentative Cost</th>
                                <th>Project</th>
                                <th>Activity Code</th>
                                <th>Account Code</th>
                                <th>Donor Code</th>
                                <th>Remarks</th>
                            </tr>
                        </thead>
                        <tbody>
                        </tbody>
                    </table>
                </div>

            </div>
        </div>
@stop
