            ...
            <div class="brd-crms flex-grow-1">
                <nav aria-label="breadcrumb">
                        <ol class="breadcrumb m-0">
                            ...
                            <li class="breadcrumb-item" aria-current="page"><a href="{!! route('daily.work.logs.index', $worklog) !!}" class="text-decoration-none text-dark">Worklog</a></li>
                            <li class="breadcrumb-item" aria-current="page">
                                <a href="{!! route('daily.work.logs.index',$worklog) !!}" class="text-decoration-none text-dark">Daily Worklog</a></li>
                        </ol>
                    </nav>
                    <h4 class="m-0 lh1 mt-1 fs-6 text-uppercase fw-bold text-primary">Add Daily Worklog</h4>
                </div>
            </div>
        </div>
        <section class="registration">
            <div class="card">
                <form class="g-3 needs-validation" action="{{ route('daily.work.logs.store', $worklog) }}" id="workPlanDailyLogAddForm" method="post"
                    ...
                    <div class="card-body">
                        ...
                        <div class="row mb-2">
                            ...
                            <div class="col-lg-9">
                                ...
                                name="major_activities">{{$workPlanDailyLog?$workPlanDailyLog->major_activities:''}}</textarea>
                        ...
                        {{--             <option value="{{ $donor->id }}" {{$donor->id == $donorId? "selected":""}}> --}}
                        ...
                        <div class="row mb-2">
                            ...
                            <div class="col-lg-9">
                                ...
                                //$donorId = '';
                                if($workPlanDailyLog){
                                    $activity_area_id = $workPlanDailyLog->activity_area_id;
                                    $priority_id = $workPlanDailyLog->priority_id;
                                    //$donorId = $workPlanDailyLog->donor_id;
                                }
                                @endphp
                                @foreach($activityAreas as $activityarea)
                                    <option value="{{ $activityarea->id }}" {{$activityarea->id == $activity_area_id? "selected":""}}>
                                        {{ $activityarea->title }}
                                    </option>
                                @endforeach
                                </select>
                        ...
                        <div class="row mb-2">
                            ...
                            <div class="col-lg-9">
                                ...
                                @foreach($priorities as $priority)
                                    <option value="{{ $priority->id }}" {{$priority->id == $priority_id? "selected":""}}>
                                        {{ $priority->title }}
                                    </option>
                                @endforeach
                                </select>
                        ...
                        <div class="row mb-2">
                            ...
                            <div class="col-lg-9">
                                <input type="text"
                                    name="status"
                                    value="{{$workPlanDailyLog?$workPlanDailyLog->status:''}}"
                                    id=""
                    ...
                    <div class="card-footer border-0 justify-content-end d-flex gap-2">
                        ...
                        <button type="submit" name="btn" value="saveandnext" class="btn btn-primary btn-sm next">Save and Next</button>
