...
class StockBookDistributionExport implements Responsable, ShouldAutoSize, WithStyles, WithStrictNullComparison, FromView
{
    use Exportable;
    ...
    private $issued_to;
    private $health_facility_id;
    private $district_id;

    public function __construct($start_date, $end_date, $item_id, $item_category, $issued_to, $health_facility_id, $district_id)
    {
    ...
    */
    private $fileName = 'stock_book_distribution_report.xlsx';
    ...
    */
    ...
    public function view(): View
    {
        ...
        if (isset($this->issued_to)) {
            $data->whereHas('distributionRequestItems', function ($q) {
                $q->whereHas('distributionRequest', function ($q) {
                    $q->where('status_id', config('constant.APPROVED_STATUS'));
        ...
        if (isset($this->health_facility_id)) {
            $data->whereHas('distributionRequestItems', function ($q) {
                $q->whereHas('distributionRequest', function ($q) {
                    $q->where('health_facility_id', $this->health_facility_id);
                });
            });
        }
        ...
        return view('Report::LogisticsProcurement.StockBook.StockBookDistribution.export', $array);
    }
}
