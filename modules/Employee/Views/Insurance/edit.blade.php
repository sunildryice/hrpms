<div class="card-header fw-bold">Edit Insurance Record<span class="training_topic"></span></div>
<form class="needs-validation" action="{{ route('employees.insurance.update', ['employee','insurance']) }}"
      method="post" id="insuranceEditForm" enctype="multipart/form-data" autocomplete="off">
    <div class="card-body">
        <div class="row mb-2">
            <div class="col-lg-3">
            ...
            <div class="col-lg-9">
                <select class="form-control @if($errors->has('payroll_fiscal_year_id')) is-invalid @endif"
                    ...
                    @foreach($payrollFiscalYears as $payrollFiscalYear)
                        <option value="{{ $payrollFiscalYear->id }}">{{ $payrollFiscalYear->title }}</option>
                    @endforeach
                </select>
    ...
    <div class="card-footer border-0 justify-content-end d-flex gap-2">
        ...
        <a onclick="cancelInsuranceEditForm(this)" class="btn btn-danger btn-sm">Cancel</a>
    </div>
    {!! csrf_field() !!}
    {!! method_field('PUT') !!}
</form>
...
<script type="text/javascript">
    document.addEventListener('DOMContentLoaded', function (e) {
        const form = document.getElementById('insuranceEditForm');
