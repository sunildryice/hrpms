    ...
    td, th {
                                ...
                                @foreach ($keygoals as $keygoal)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <span style="width: 100%">{{$keygoal->description_employee}}</span>

                                        </td>
                                        <td>
                                            <textarea style="width: 100%; height: 100%" name="{{'keygoal_supervisor_'.$keygoal->id}}" id="{{'keygoal_supervisor_'.$keygoal->id}}" rows="2">{{$keygoal->description_supervisor}}</textarea>
                            ...
                            @foreach ($groupEQuestions as $question)
                                @if (!$loop->last)
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-lg-5">
                                            <textarea name="{{'question_'.$question->id}}" id="{{'question_'.$question->id}}" style="width: 100%;" rows="3">{{$performanceReview->getAnswer($question->id)}}</textarea>
                                        </div>
                                    </div>
                                @endif
                            @endforeach
        ...
        <section>
            <div class="card">
                ...
                <form action="{{route('performance.review.store')}}"
                                ...
                                <div class="row mb-2">
                                    ...
                                    <div class="col-lg-9">
                                        <select name="status_id" class="select2 form-control" data-width="100%">
                                            ...
                                            <option value="{{ config('constant.APPROVED_STATUS') }}" {{old('status_id') == config('constant.APPROVED_STATUS') ? 'selected' : ''}}>Approve</option>
                                        </select>
                                    ...
                                    <div class="col-lg-9">
                                        <select name="receiver_id" id="receiver_id" class="select2 form-control" data-width="100%">
                                           @foreach ($receivers as $receiver)
                                                @if ($loop->first)
                                                    <option value="{{$receiver->id}}" {{old('receiver_id') == $receiver->id ? 'selected' : (empty(old('receiver_id')) ? 'selected' : '')  }}>{{$receiver->getFullName()}}</option>
                                                @else
                                                    <option value="{{$receiver->id}}">{{$receiver->getFullName()}}</option>
                                                @endif
                                           @endforeach
                                        </select>
