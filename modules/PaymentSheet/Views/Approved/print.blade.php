            ...
            <div class="col-lg-8">
                ...
                <div class="mb-3 print-header-info">
                    <ul class="p-0 m-0 list-unstyled fs-7">
                        ...
                        <li><span class="fw-bold me-2">Bank A/C Name :
                            </span><span>{{ $paymentSheet->supplier->account_name }}</span></li>
    ...
    <div class="mb-5 print-body">
        ...
        <div class="row">
            <div class="mb-4 col-lg-6">
                ...
                <div><strong>Title:</strong> {{ $paymentSheet->submittedLog->getDesignation() }} </div>
            ...
            <div class="mb-4 col-lg-6">
                ...
                <div><strong>Title:</strong> {{ $paymentSheet->getVerifierDesignation() }}</div>
            ...
            <div class="mb-4 col-lg-6">
                ...
                <div><strong>Title:</strong> {{ $paymentSheet->getRecommenderDesignation() }}</div>
                <div><strong>Date:</strong> {{ $paymentSheet->getRecommendedDate() }}</div>
            </div>
            @isset($paymentSheet->reviewer_id)
                <div class="mb-4 col-lg-6">
                    <div><strong>Reviewed By:</strong></div>
                    <div><strong>Name:</strong> {{ $paymentSheet->reviewer?->getFullName() }}</div>
                    <div><strong>Title:</strong> {{ $paymentSheet->getReviewerDesignation() }}</div>
                    <div><strong>Date:</strong> {{ $paymentSheet->getReviewedDate() }}</div>
                </div>
            @endisset
            <div class="mb-4 col-lg-6">
                <div><strong>Approved By:</strong></div>
                <div><strong>Name:</strong> {{ $paymentSheet->approver->getFullName() }} </div>
                <div><strong>Title:</strong> {{ $paymentSheet->getApproverDesignation() }} </div>
                <div><strong>Date:</strong> {{ $paymentSheet->getApprovedDate() }} </div>
            </div>
            @if (isset($paymentSheet->paid_at))
                <div class="mb-4 col-lg-6">
                    <div><strong>Paid By:</strong></div>
                    <div><strong>Name:</strong> {{ $paymentSheet->getPayerName() }} </div>
                    <div><strong>Title:</strong> {{ $paymentSheet->getPayerDesignation() }} </div>
                    <div><strong>Date:</strong> {{ $paymentSheet->pay_date->format('M d, Y') }} </div>
                </div>
            @endif
        </div>
    </div>
