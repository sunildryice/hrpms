...
class PendingController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $data = $this->attendances->getPending();
            ...
            ->addColumn('employee_code', function ($row) {
                return $row->employee->getEmployeeCode();
            })
            ->addColumn('supervisor', function ($row) {
                return $row->employee->latestTenure->getSupervisorName();
            })
            ->addColumn('duty_station', function ($row) {
                return $row->employee->latestTenure->getDutyStation();
            })
        ...
        return view('EmployeeAttendance::Pending.index');
    }
}