...
<form class="g-3 needs-validation" action="{{ route('employees.tenures.store', $employee) }}" method="post"
      id="tenureAddForm" enctype="multipart/form-data" autocomplete="off">
    <div class="card-body">
        @php
        ...
        <div class="row mb-2">
            <div class="col-lg-3">
                <div class="d-flex align-items-start h-100">
                    <label for="Fdname" class="m-0">Designation *</label>
                </div>
            </div>
            <div class="col-lg-9">
                <select name="designation_id"
                    ...
                    @foreach($designations as $designation)
                        <option value="{{ $designation->id }}"
                                @if($selectedDesignationId == $designation->id) selected @endif>{{ $designation->getDesignationName() }}</option>
                    @endforeach
                </select>
        ...
        <div class="row mb-2">
            <div class="col-lg-3">
                <div class="d-flex align-items-start h-100">
                    <label for="Fdname" class="m-0">Department *</label>
                </div>
            </div>
            <div class="col-lg-9">
                <select name="department_id"
                    ...
                    @foreach($departments as $department)
                        <option value="{{ $department->id }}"
                                @if($selectedDepartmentId == $department->id) selected @endif>{{ $department->getDepartmentName() }}</option>
                    @endforeach
                </select>
        ...
        <div class="row mb-2">
            <div class="col-lg-3">
                <div class="d-flex align-items-start h-100">
                    <label for="validationdob" class="m-0">Joined Date *</label>
                </div>
            </div>
            <div class="col-lg-9">
                <input type="text" class="form-control @if($errors->has('joined_date')) is-invalid @endif"
                       name="joined_date" placeholder="Joined Date"
                       value="{{ old('joined_date') ?: ($employee->latestTenure->joined_date ? $employee->latestTenure->joined_date->format('Y-m-d') : '') }}" readonly/>
        ...
        <div class="row mb-2">
            <div class="col-lg-3">
                <div class="d-flex align-items-start h-100">
                    <label for="validationdob" class="m-0">Supervisor</label>
                </div>
            </div>
            <div class="col-lg-9">
                <select name="supervisor_id"
                    ...
                    @foreach($supervisors as $supervisor)
                        <option value="{{ $supervisor->id }}"
                                @if($selectedSupervisorId == $supervisor->id) selected @endif>{{ $supervisor->getFullName() }}</option>
                    @endforeach
                </select>
        ...
        <div class="row mb-2">
            <div class="col-lg-3">
                <div class="d-flex align-items-start h-100">
                    <label for="validationdob" class="m-0">Cross Supervisor</label>
                </div>
            </div>
            <div class="col-lg-9">
                <select name="cross_supervisor_id"
                        class="select2 form-control @if($errors->has('cross_supervisor_id')) is-invalid @endif" data-width="100%">
                    <option value="">Select a Cross Supervisor</option>
                    @foreach($supervisors as $supervisor)
                        <option value="{{ $supervisor->id }}"
                                @if($selectedCrossSupervisorId == $supervisor->id) selected @endif>{{ $supervisor->getFullName() }}</option>
                    @endforeach
                </select>
        ...
        <div class="row mb-2">
            <div class="col-lg-3">
                <div class="d-flex align-items-start h-100">
                    <label for="validationcitizenship" class="m-0">Next Line Manager</label>
                </div>
            </div>
            <div class="col-lg-9">
                <select name="next_line_manager_id"
                        class="select2 form-control @if($errors->has('next_line_manager_id')) is-invalid @endif" data-width="100%">
                    <option value="">Select a Next Line Manager</option>
                    @foreach($supervisors as $supervisor)
                        <option value="{{ $supervisor->id }}"
                                @if($selectedNextLineManagerId == $supervisor->id) selected @endif>{{ $supervisor->getFullName() }}</option>
                    @endforeach
                </select>
        ...
        <div class="row mb-2">
            <div class="col-lg-3">
                <div class="d-flex align-items-start h-100">
                    <label for="validationcitizenship" class="m-0">Duty Station</label>
                </div>
            </div>
            <div class="col-lg-9">
                <select name="duty_station_id"
                        class="select2 form-control @if($errors->has('duty_station_id')) is-invalid @endif" data-width="100%">
                    <option value="">Select a Duty Station</option>
                    @foreach($districts as $district)
                        <option value="{{ $district->id }}"
                                @if($selectedDutyStationId == $district->id) selected @endif>{{ $district->getDistrictName() }}</option>
                    @endforeach
                </select>
        ...
        <div class="row mb-2">
            ...
            <div class="col-lg-9">
                <textarea name="remarks" class="form-control"
                          placeholder="Remarks">{!! old('remarks') ?: $employee->latestTenure->remarks !!}</textarea>
            </div>
        </div>
    </div>
            ...
            const fv = FormValidation.formValidation(form, {
                fields: {
                    ...
                    cross_supervisor_id: {
                        validators: {
                            // notEmpty: {
                            //     message: 'Cross supervisor is required',
                            // },
                            different: {
                                compare: function () {
                    ...
                    next_line_manager_id: {
                        validators: {
                            // notEmpty: {
                            //     message: 'Next line manager is required',
                            // },
                            different: {
                                compare: function () {
