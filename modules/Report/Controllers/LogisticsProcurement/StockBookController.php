...
use Modules\Report\Exports\LogisticsProcurement\StockBookExport;
...
class StockBookController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        //             $q->where('fiscal_year_id', $request->fiscal_year);
        //         });
        //     });
        //     $data->orWhereHas('distributionRequestItems', function ($q) use($request) {
        ...
        if ($request->filled('issued_to')) {
            $data->whereHas('goodRequestItems', function ($q) use ($request) {
                $q->whereHas('goodRequest', function ($q) use ($request) {
                    ...
                    $q->where('created_by', $request->issued_to);
                });
            });
            $data->orWhereHas('distributionRequestItems', function ($q) use ($request) {
                $q->whereHas('distributionRequest', function ($q) use ($request) {
        ...
        return view('Report::LogisticsProcurement.StockBook.index', $array);
    }
    ...
    public function export(Request $request)
    {
        ...
        return new StockBookExport($start_date, $end_date, $item_id, $item_category, $issued_to);
    }
}
