...
use Modules\PurchaseRequest\Notifications\Procurementtest;
...
use Modules\PurchaseRequest\Requests\Special\UpdateRequest as SpecialUpdateRequest;
...
class PurchaseRequestController extends Controller
{
    ...
    public function index(Request $request)
    {
        $authUser = auth()->user();
        if ($request->ajax()) {
            $data = $this->purchaseRequests->with(['fiscalYear', 'status', 'logs', 'purchaseRequestItems', 'purchaseOrderItems', 'grns'])->select(['*'])
    ...
    {
        $authUser = auth()->user();
        $officers = $this->users->permissionBasedUsers('manage-procurement');
        return view('PurchaseRequest::create')->withOfficers($officers)
    ...
    public function amend($id)
    {
        ...
        $clone = $this->purchaseRequests->amend($id, $inputs);
        if ($clone) {
            return response()->json([
                'type' => 'success',
                'message' => 'Purchase request is successfully amended.',
                'redirectUrl'=>route('purchase.requests.edit', $clone->id),
            ], 200);
        }
        return response()->json([
            'type' => 'error',
    ...
    public function specialIndex(Request $request)
    {
        $authUser = auth()->user();
        if ($request->ajax()) {
            $data = $this->purchaseRequests->get();
            ...
            return DataTables::of($data)
                ...
                ->addColumn('required_date', function ($row) {
                    ...
                    if($authUser->can('specialUpdate', $row)) {
                        $btn .= '&emsp;<a class="btn btn-outline-primary btn-sm" href="';
                        $btn .= route('purchase.requests.special.edit', $row->id) . '" rel="tooltip" title="Edit Purchase Request"><i class="bi-pencil-square"></i></a>';
                    }
        ...
        return view('PurchaseRequest::Special.index');
    }
    ...
    public function specialEdit($id)
    {
        ...
        return view('PurchaseRequest::Special.edit')
            ->withAuthUser($authUser)
            ->withPurchaseRequest($purchaseRequest);
    }

    public function specialUpdate(SpecialUpdateRequest $request, $id)
    {
        $purchaseRequest = $this->purchaseRequests->find($id);
