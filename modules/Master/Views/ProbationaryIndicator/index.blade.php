...
@section('title', __('label.probationary-indicators'))
...
@section('page_js')
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#navbarVerticalMenu').find('#probationary-indicators-menu').addClass('active');

            var oTable = $('#probationaryIndicatorTable').DataTable({
                scrollX: true,
                processing: true,
                serverSide: true,
                ajax: "{{ route('master.probationary.indicators.index') }}",
                columns: [{
                        data: 'title',
                        name: 'title',
                        orderable: false
            ...
            $(document).on('click', '.open-indicator-modal-form', function(e) {
                e.preventDefault();
                $('#openModal').find('.modal-content').html('');
                $('#openModal').modal('show').find('.modal-content').load($(this).attr('href'), function (){
                    const form = document.getElementById('probationaryIndicatorForm');
                    const fv = FormValidation.formValidation(form, {
                        fields: {
                            title: {
                                validators: {
                                    notEmpty: {
                                        message: 'Probationary indicator is required',
                                    },
                                },
                            },
                        },
        ...
        <div class="pb-3 mb-3 border-bottom">
            ...
            <div class="brd-crms flex-grow-1">
                ...
                <div class="add-info justify-content-end">
                    <button data-toggle="modal" class="btn btn-primary btn-sm open-indicator-modal-form"
                        href="{!! route('master.probationary.indicators.create') !!}">
                    ...
                    <div class="table-responsive">
                        <table class="table" id="probationaryIndicatorTable">
                            <thead class="thead-light">
                                <tr>
                                    <th scope="col">{{ __('label.probationary-indicator') }}</th>
