...
use Modules\AdvanceRequest\Requests\Settlement\Verify\StoreRequest;
use DataTables;


class VerifySettlementController extends Controller
{
    /**
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('requester', function ($row) {
                    ...
                    $btn .= route('verify.advance.settlements.create', $row->id) . '" rel="tooltip" title="Verify Settlement  Request">';
        ...
        return view('AdvanceRequest::Settlement.Verify.index');
    }
    ...
    public function create($advanceSettlementId)
    {
        ...
        $this->authorize('verify', $advanceSettlement);

        return view('AdvanceRequest::Settlement.Verify.create')
    ...
    public function store(StoreRequest $request, $advanceSettlementId)
    {
        ...
        $advanceSettlement = $this->settlements->verify($advanceSettlement->id, $inputs);
        if ($advanceSettlement) {
            ...
            return redirect()->route('verify.advance.settlements.index')
                ->withSuccessMessage($message);
        }

        return redirect()->back()
            ->withInput()
            ->withWarningMessage('Advance request settlement can not be verified.');
    }
}
