                ...
                <div class="row">
                    ...
                    <div class="col-lg-9">
                        <div class="card">
                            ...
                            <form action="{!! route('training.report.update',$trainingReport->id) !!}" method="post"
                                ...
                                <div class="card-body">
                                    @if($trainingReportQuestions->count()>0)
                                        @foreach($trainingReportQuestions as $trainingReportQuestion)
                                            @if($trainingReportQuestion->trainingQuestion->answer_type == 'textarea')
                                                <div class="row mb-2">
                                                    <div class="col-lg-12">
                                                        <label for="qone" class="form-label required-label">{{$trainingReportQuestion->trainingQuestion->question}}
                                                        </label>
                                                        <textarea name="textarea[{{$trainingReportQuestion->trainingQuestion['id']}}]" id="qone" cols="30" rows="5" class="form-control question
                                                            @if($errors->has('textarea[{{$trainingReportQuestion->trainingQuestion["id"]}}]')) is-invalid @endif">{{$trainingReportQuestion->answer}}</textarea>
                                                        @if($errors->has('textarea[{{$trainingReportQuestion->trainingQuestion["id"]}}]'))
                                                            <div class="fv-plugins-message-container invalid-feedback">
                                                                <div data-field="textarea[{{$trainingReportQuestion->trainingQuestion['id']}}]">
                                                                    {!! $errors->first('textarea[{{$trainingReportQuestion->trainingQuestion["id"]}}]') !!}
                                                                </div>
                                                            </div>
                                                        @endif
                                                    </div>
