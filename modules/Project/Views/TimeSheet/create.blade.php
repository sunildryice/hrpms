            ...
            if (form) {
                ...
                function initRow($row) {
                    ...
                    $row.find('.project-select').on('change', function() {
                        ...
                        if (projectId) {
                            var url = "{{ route('api.projects.show', ['projectId']) }}".replace('projectId',
                                projectId);
                            ...
                            var successCallback = function(response) {
                                ...
                                $($obj).closest('tr').find('.activity-select').html(
                                    htmlToReplaceActivity);
                ...
                @if ($minAllowedDate)
                    datepickerOptions.startDate = '{{ $minAllowedDate }}';
                @endif

                $('[name="timesheet_date"]').datepicker(datepickerOptions)
                    .on('change', function() {
                        fv.revalidateField('timesheet_date');
                    });

                // Set default to today (safe because today <= endDate and >= startDate if set)
                if (!$('[name="timesheet_date"]').val().trim()) {
                    const today = '{{ $maxAllowedDate }}'; 
