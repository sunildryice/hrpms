...
class UpdateRequest extends FormRequest
{
    ...
    {
        // $paidAmount = PaymentSheetDetail::select('net_amount')
        // ->where('payment_sheet_id', $this->paymentSheet)
        // ->where('payment_bill_id', request()->payment_bill_id)
        // ->where('id', '!=', $this->detail)
        // ->sum('total_amount');

        // $paymentBill = PaymentBill::find(request()->payment_bill_id);
        // $maxAmount = floatval($paymentBill->total_amount-$paidAmount);

        $paymentBill = PaymentBill::find(request()->payment_bill_id);

        $paidAmount = PaymentSheetDetail::where('payment_bill_id', request()->payment_bill_id)
        ->where('id', '!=', $this->detail)
        ->sum('total_amount');
