...
@section('title', 'Add New Encashment Request')
...
@section('page_js')
    <script type="text/javascript">
        ...
        var leaveTypeId = '{{ old('leave_type_id') }}'
        var employeeId = '12'
        ...
        document.addEventListener('DOMContentLoaded', function(e) {
            ...
            // if ($('[name="leave_type_id"]').val()) {
            //     var leave = leaveTypes.filter(element => {
            //         return element.leave_type_id == $('[name="leave_type_id"]').val();
            //     })[0];
            //     if (leave) {
            ...
            //         $('[name="available_balance"]').val(leave.balance);
            ...
            if ($('[name="employee_id"]').val()) {
                var eId = $('[name="employee_id"]').val();
                var field = '<option value="">Select a Leave Type</option>';
                if (employeeId) {
                    var url = baseUrl + '/api/employee/' + eId + '/leaves/fetch';
                    var successCallback = function(response) {
                        leaveTypes = response.leaveTypes;
                        $('[name="balance"]').val(null);
                        $('[name="available_balance"]').val(null);
                        ...
                        $('[name="leave_type_id"]').html(field).trigger('change');
                    }
            ...
            $(form).on('change', '[name="employee_id"]', function(e) {
                ...
                if (employeeId) {
                    ...
                    var successCallback = function(response) {
                        console.log(response.leaveTypes[0].reported_date);
                        leaveTypes = response.leaveTypes;
                        ...
                        response.leaveTypes.forEach(leave => {
                            field += '<option value="' + leave.leave_type.id +
                                '" data-leave="' + leave.id +
                                '">' + leave.leave_type.title + '</option>';
                        });
                        $(ele).closest('form').find('[name="leave_type_id"]').html(field);
                    }
                    var errorCallback = function(error) {
                        console.log(error);
                ...
                if (leaveTypeId) {
                    ...
                    //     '/show';
                    // var successCallback = function(response) {
                    //     var balance = 'Available Balance : ' + response.leave.balance + ' - ' + response
                    //         .leaveBasis;
                    //     $($element).closest('form').find('[name="balance"]').val(balance);
                    //     $($element).closest('form').find('[name="available_balance"]').val(response
                    //         .leave.balance);
...
@section('page-content')
    ...
    <section class="registration">
        <div class="card shadow-sm border-0 rounded">
            <form action="{{ route('leave.encash.store') }}" id="leaveEncashAddForm" method="post"
                        ...
                        <div class="col-lg-7 mb-3 mb-lg-0">
                            <select name="leave_type_id" class="select2 form-control" data-width="100%">
                                ...
                                {{-- @foreach ($leaveTypes as $leave)
                                    <option value="{{ $leave->leaveType->id }}" data-leave="{{ $leave->id }}"
                                        {{ $leave->leaveType->id == old('leave_type_id') ? 'selected' : '' }}>
                    ...
                    <div class="row
                        ...
                        <div class="col-lg-10">
                            <input type="number" class="form-control @if ($errors->has('remarks')) is-invalid @endif"
                                name="encash_balance" value="{{ old('encash_balance') }}" />
                            ...
                            <select name="reviewer_id"
                                ...
                                @foreach ($reviewers as $reviewer)
                                    <option value="{{ $reviewer->id }}"
                                        {{ $reviewer->id == old('reviewer_id') ? 'selected' : '' }}>
