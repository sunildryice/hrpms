...
@php
$action = $employee->user ? route('consultant.user.update', $employee->id) : route('consultant.user.store', $employee->id);
$selectedRoleIds = $employee->user ? $employee->user->roles->pluck('id')->toArray() : [];
@endphp
        ...
        <div class="row mb-2">
            <div class="col-lg-3">
                <div class="d-flex align-items-start h-100">
                    <label for="username" class="form-label">Official Email</label>
                </div>
            </div>
            <div class="col-lg-9">
                <input type="text" class="form-control @if ($errors->has('email_address')) is-invalid @endif"
                    name="email_address" value="{{ $employee->official_email_address ?: $employee->user?->email_address }}"  readonly/>
