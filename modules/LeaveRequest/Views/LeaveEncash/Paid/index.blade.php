...
@section('title', 'Paid Leave Encash Requests')

@section('page_js')
    <script type="text/javascript">
        $(document).ready(function() {
            $('#navbarVerticalMenu').find('#paid-leave-encash-menu').addClass('active');

            var oTable = $('#leaveEncashTable').DataTable({
                scrollX: true,
                processing: true,
                serverSide: true,
                ajax: "{{ route('paid.leave.encash.index') }}",
                columns: [{
                        data: 'DT_RowIndex',
                        name: 'DT_RowIndex',
                        orderable: false,
                    ...
                    <table class="table" id="leaveEncashTable">
                        <thead class="bg-light">
                            <tr>
                                ...
                                <th>{{ __('label.encash-balance') }}</th>
                                <th>{{ __('label.request-date') }}</th>
                                <th>{{ __('label.encash-number') }}</th>
