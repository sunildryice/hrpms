...
use Modules\Payroll\Requests\Review\StoreRequest;
...
class BatchReviewController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('created_by', function ($row) {
                    ...
                    $btn .= route('payroll.batches.review.create', $row->id) . '" rel="tooltip" title="Verify Payroll Batch"><i class="bi bi-hand-thumbs-up"></i></a>';
                    return $btn;
                })->rawColumns(['action', 'status'])
                ->make(true);
        }
        return view('Payroll::Batch.Review.index');
    }
    ...
    public function create($id)
    {
        ...
        $authUser->can('review', $payrollBatch);

        return view('Payroll::Batch.Review.create')
    ...
    public function store(StoreRequest $request, $id)
    {
        ...
        $payrollBatch = $this->payrollBatches->review($id, $inputs);

        if ($payrollBatch) {
            return redirect()->route('payroll.batches.review.index')
                ->withSuccessMessage('Payroll batch successfully reviewed.');
        }
        return redirect()->back()->withInput()
            ->withWarningMessage('Payroll batch can not be reviewed.');
    }
}
