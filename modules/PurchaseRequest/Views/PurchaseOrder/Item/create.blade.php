...
<form action="{{route('approved.purchase.requests.orders.editItem',$purchaseOrder->id)}}" method="post" enctype="multipart/form-data" id="purchaseOrderCombineForm"
    autocomplete="off">
    @csrf
        ...
        <div class="row mb-2">
            ...
            <div class="col-lg-9">
                <select class="form-control select2" data-width="100%" name="purchase_request_id">
                    ...
                    @foreach ($prIds as $po)
                        <option value="{!! $po->id !!}">{{ $po->getPurchaseRequestNumber() }}</option>
                    @endforeach
                </select>
            </div>
        </div>
