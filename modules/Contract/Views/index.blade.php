...
@section('title', __('label.contracts'))
        ...
        $(document).ready(function () {
            ...
            var oTable = $('#contractTable').DataTable({
                ...
                columns: [
                    ...
                    {data: 'description', name: 'description', className: 'wrap-col'},
            ...
            $('#contractTable').on('click', '.delete-record', function (e) {
                e.preventDefault();
                $object = $(this);
                ...
                $('#openModal').modal('show').find('.modal-content').load($(this).attr('href'), function (){
                            ...
                            reminder_days: {
                                validators: {
                                    between: {
                                        min: 1,
                                        max: 90,
                                        message: 'The reminder days must be be between 1 and 90',
                                    },
                                },
                            },
                            termination_days: {
                                validators: {
                                    between: {
                                        min: 1,
                                        max: 90,
                                        message: 'The termination days must be be between 1 and 90',
                                    },
                                },
                            },
                            attachment: {
                    ...
                    $(form).on('change', '[name="supplier_id"]', function (e) {
                        ...
                        if (supplierId) {
                            ...
                            var successCallback = function (response) {
                                $($element).closest('form').find('[name="contact_name"]').val(response.supplier.contact_person_name);
                                $($element).closest('form').find('[name="contact_number"]').val(response.supplier.contact_number);
                                $($element).closest('form').find('[name="address"]').val(response.supplier.address1);
                            }
        ...
        <div class="pb-3 mb-3 border-bottom">
            ...
            <div class="brd-crms flex-grow-1">
                ...
                <div class="add-info justify-content-end">
                    <button data-toggle="modal" class="btn btn-primary btn-sm open-contract-modal-form"
                            href="{!! route('contracts.create') !!}">
                        <i class="bi-plus"></i> Add New
                    </button>
                    <a class="btn btn-secondary btn-sm" title="Export Contracts to Excel"
                    ...
                    <div class="table-responsive">
                        <table class="table" id="contractTable">
                            ...
                            <tr>
                                ...
                                <th>{{ __('label.contract-description') }}</th>
                                <th>{{ __('label.contract-amount') }}</th>
                                <th>{{ __('label.contract-date') }}</th>
                                <th>{{ __('label.effective-date') }}</th>
